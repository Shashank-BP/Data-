...
from online_retail;
-- Result:Total_value=541909,InvoiceNo_missing=0,StockCode_missing=0,Description_missing=1445,Quantity_missing=0,InvoiceDate_missing=0,UnitPrice_missing=0,CustomerID_missing=135080,Country_missing=0

-- Removing the null values
delete from online_retail where CustomerID = 0;
/* While looking at the number of null values in the dataset, it is interesting to note that almost 25% of the entries are not assigned to a particular customer. With the data available, 
it is impossible to impute values for the user and these entries are thus useless for the current exercise. So I delete them from the dataset. */
...
Result: Total sales in each country are United Kingdom=6525542.84, Netherland=273346.98, EIRE=250447.48, Germany=220093.16, France=194765.68, Australia=134453.20, Spain=57412.76, Switzerland=55678.20, Belgium=40910.96,
		Sweden=36574.91, Norway=34443.56, Portugal=28750.62, Japan=26176.22, Finland=21521.14, Channel Islands=20066.49, Denmark=18581.89, Italy=16590.51, Cyprus=12579.33, Austria=10154.32, Singapore=9120.39,
		Poland=7213.14, Israel=6982.55, Greece=4710.52, Iceland=4310.00, Unspecified=2640.77, Malta=2505.47, United Arab Emirates=1902.28, USA=1730.92, Lebanon=1693.88, Lithuania=1661.06, European Community=1291.75,
		Brazil=1143.60, RSA=1002.31, Czech Republic=707.72, Saudi Arabia=131.17.
*/
...
Resutl: The total sales for each hour are 6=-497.35, 7=29376.84, 8=265859.47, 9=651293.74, 10=1110274.64, 11=1013277.00, 12=1288293.27, 13=1080950.30, 14=936643.41, 15=868288.44, 16=426855.25, 
...
Results: The total sales for each month are January=439766.80, February=423042.99, March=566197.03, April=415802.32, May=637596.35, July=578894.89, September=899674.81, November=1084676.37, December=843284.74,
*/
...
from online_retail;
/* 
Query summary: This query counts the number of invoices that start with the letter 'C' and calculates the percentage of total invoices that are cancellations. The case statement is used to count 
			   only the invoices that start with 'C' and return null for all other invoices. The count function then counts the number of non-null values returned by the case statement. 
Result: Based on the results we foound that there is a large percentage of cancelled orders which is about 40%. More analysis on the cancelled orders can help in preventing future cacellation.
*/
...
limit 10;
/*
Query summary: This query will group the sales data by StockCode and Description, and calculate the total quantity and sales for each item. 
			   It will then sort the results by total quantity and sales in descending order, and limit the output to the top 10 items.
Results: The top-selling items in the online retail dataset include Jumbo Bag Red Retrospot, World War 2 Gliders Asstd Designs, and White Hanging Heart T-Light Holder. 
...
/* Customer segmentation is the process of dividing a customer base into distinct groups of individuals that have similar characteristics. This process makes it easier to target specific groups 
of customers with tailored products, services, and marketing strategies. By segmenting customers into different classes, businesses can better understand their needs, preferences, and 
buying patterns, allowing them to create more personalized and effective marketing campaigns. */
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
Results: Based on the customer segmentation analysis, the majority of customers in the online retail dataset are from the United Kingdom. This information can be used to tailor 
...
Results: Based on the results, customers can be segmented into three categories: high-spending countries (UK, France, and Australia), moderate-spending countries (Netherlands, Germany, EIRE, and Switzerland),
...
group by CustomerID;
/*
Result: Based on the query result, we can observe that the customers have a wide range of total spending amounts, varying from negative values to a maximum of 268174.46. This information can 
...
group by CustomerID;
/*
Result: The query results show a wide range in the amount that customers spend on average, indicating potential different customer segments based on purchasing power and interests. 
...
group by CustomerID;
/*
Result: The above query helps to segment customers based on their purchase frequency between a certain time period. From the result, it can be observed that the frequency of purchases ranges 
...
/*
Query summary: * High Spenders: Customers who have spent more than 1000 have been identified as high spenders.
...
Result: From the results of the query, we can see that the majority of customers (1990) fall into the "Occasional Buyers" category, followed by "High Spenders" (1588), "Regular Buyers" (666), 
...
/*
Conclusion: In summary, the online retail dataset contains information about transactions of a company with 21982 unique transactions made by 4356 customers across different countries. The dataset 
...
/*
Note: The customer ID column in the dataset had up to 25% null values, which were removed prior to the analysis and segmentation. As a result, the results presented in this analysis are based on a reduced 
	  dataset and may not be representative of the entire population. Additionally, the analysis was limited to a specific date range, and assumptions were made regarding customer behavior. These limitations 
